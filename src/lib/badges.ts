...
import { computeQualityScore, FRAME_CERTIFIED_THRESHOLD } from "./quality";
 ...
 * video that doesn't actually play back that wide. `FRAME Certified` is the
...
export function computeBadges(video: Video): Badge[] {
  ...
  if (computeQualityScore(video) >= FRAME_CERTIFIED_THRESHOLD) badges.add("FRAME Certified");
